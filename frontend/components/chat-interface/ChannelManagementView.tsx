...
import {
  getChannelManagementBootstrapActions,
...
export function ChannelManagementView({
  ...
  const qrcodeRenderState = getChannelManagementQrRenderState({
    qrcode: qrcodePayload,
    qrcodeUrl,
    generatedDataUrl: qrcodeDataUrl,
  })
  ...
  useEffect(() => {
    const actions = getChannelManagementBootstrapActions(userId)
    if (actions.loadBoundUsers) {
      void loadBoundUsers()
    }
    if (actions.generateQrcodeUserId) {
      void generateQrcode(actions.generateQrcodeUserId)
    }
  }, [generateQrcode, loadBoundUsers, userId])

  useEffect(() => {
    if (!qrcodeRenderState.shouldGenerateDataUrl) {
      setQrcodeDataUrl('')
      return
    }

    let cancelled = false
    void QRCode.toDataURL(qrcodeRenderState.payload, {
  ...
  }, [qrcodeRenderState.payload, qrcodeRenderState.shouldGenerateDataUrl])
      ...
      <div className={styles.managementPageGrid}>
        <section className={styles.managementCard}>
          ...
          <div className={styles.channelQrPanel}>
            {qrcodeRenderState.imageSrc ? (
              <img
                className={styles.channelQrImage}
                src={qrcodeRenderState.imageSrc}
