...
def test_channels_router_is_importable_from_nested_business_api_package():
    from langchain_api.api.channels.api.routes import create_channels_router
    assert callable(create_channels_router)

def test_channels_schema_is_importable_from_business_schema_package():
    from langchain_api.api.channels.schemas.weixin_clawbot import WeixinClawBotPollRequest
...
def test_weixin_clawbot_user_qrcode_routes_persist_user_runtime_state(monkeypatch):
    ...
    client = TestClient(app)
    monkeypatch.setattr(
        "langchain_api.api.channels.api.routes.start_weixin_clawbot_runtime",
        fake_start_runtime,
    )
...
def test_weixin_clawbot_user_management_lists_and_deletes_bound_users(monkeypatch):
    ...
    client = TestClient(app)
    monkeypatch.setattr(
        "langchain_api.api.channels.api.routes.stop_weixin_clawbot_runtime",
        fake_stop_runtime,
    )
