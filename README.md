...
LangChain API 是一个面向二次开发的智能体服务。项目把通用 Agent、RAG 知识库、技能管理、渠道接入、MCP 配置和静态前端整合到同一个 FastAPI 服务中，适合快速搭建企业知识问答、自动化助手、内部 Copilot 和多工具智能体应用。
...
langchain-api/
├── langchain_api/
│   ├── agent/              # 通用 Agent 组装、上下文、技能管理
│   ├── api/                # FastAPI 路由注册、SSE 端点、管理接口
│   ├── channels/           # 飞书、钉钉、微信 ClawBot 渠道接入
│   ├── middleware/         # 业务开关、RAG 注入、MCP、工具搜索等中间件
│   ├── rag/                # 知识库管理、ES 检索、Graph RAG
