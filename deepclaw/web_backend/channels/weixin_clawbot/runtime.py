...
class WeixinClawBotRuntime:
    ...
    async def _load_runtime_state(self) -> dict[str, Any]:
        if self.store is None:
            return {}
        state = await self.store.get_runtime_state(
            channel=CHANNEL,
            state_key=self.state_key,
        )
        return dict(state.data) if state is not None and state.data else {}

    async def _load_binding(self):
        if self.store is None or self.binding_id is None:
            return None
        return await self.store.get_binding(self.binding_id)
    ...
    async def _save_runtime_state(self) -> None:
        ...
        await self._save_binding_state(
            status="connected" if self.bot_token else "pending",
        )

    async def _save_binding_state(self, *, status: str) -> None:
